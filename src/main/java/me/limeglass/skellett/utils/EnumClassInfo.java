...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public ClassInfo<E> register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Skellett.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Skellett.getConfiguration("syntax").set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enumType.getEnumConstants());
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Skellett.getInstance().getConfig().getString("syntaxDebug"))) Skellett.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Skellett.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Skellett.getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
